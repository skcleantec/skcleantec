...
금융·건강·사진·동영상·파일·캘린더·**연락처(주소록)**·위치·웹 브라우징·광고 ID·SMS 본문

> 기기 **연락처 주소록**은 읽지 않습니다. CRM에서 받은 고객 번호만 표시합니다.
...
**전화번호·통화기록:** 기기 CallLog에서 읽어 `POST /api/crm/call-sessions/sync` 로 **자사 서버**에만 전송 (판매·광고 목적 없음).
