...
export const OUTBOUND_EMAIL_PROVIDERS: OutboundEmailProviderPreset[] = [
  {
    ...
    passwordSteps: [
      ...
      '보안 → 「앱 비밀번호」에서 메일용 16자리 비밀번호를 발급합니다.',
      '아래에 공백 없이 16자리를 입력합니다.',
    ],
  },
  {
    id: 'naver',
    ...
    port: '587',
    secure: false,
    shortNote: '네이버 메일 환경설정에서 SMTP 사용 ON',
    passwordHint: '네이버 메일 → 환경설정 → POP3/IMAP → SMTP 사용',
    passwordSteps: [
      '네이버 메일 → 환경설정 → POP3/IMAP 설정에서 SMTP 사용을 켭니다.',
      '네이버 로그인 비밀번호를 아래에 입력합니다.',
    ],
  },
  {
    id: 'daum',
