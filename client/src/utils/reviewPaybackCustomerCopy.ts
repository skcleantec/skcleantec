...
export function buildReviewPaybackMessageBlock(paybackLink: string, blockTemplate?: string): string {
  const template =
    blockTemplate?.trim() ||
    `★ 리뷰 페이백 신청 (전화·카톡 확인 없이 링크에서만 접수됩니다)
리뷰 작성 후 반드시 아래 링크에 접속해 캡처·계좌를 등록해 주세요.
전화나 메시지로 보내주시면 확인이 지연될 수 있습니다.

페이백 신청: {{paybackLink}}`;
  return template.replace(/\{\{paybackLink\}\}/g, paybackLink);
}
