  ...
  return (
    <div ref={wrapRef} className={`relative group inline-flex ${className ?? ''}`}>
      <button
        ...
        aria-label={'\uB3C4\uC6C0\uB9D0'}
        onClick={() => setOpen((p) => !p)}
        className="inline-flex h-5 w-5 items-center justify-center rounded-full border border-gray-300 bg-white text-[12px] font-semibold text-gray-600 hover:bg-gray-50"
      >
        ?
      </button>
      <div
