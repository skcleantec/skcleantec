...
export function CrmCallMemoPanel({
  phone,
  inquiryId,
  resetKey = 0,
}: {
  ...
  return (
    <CrmColumn accent="intake" title="통화 메모" subtitle="상담 내용 기록 · 고객에게 보내지 않음">
      <div className="space-y-3">
        <label className="block space-y-1">
          <span className="text-fluid-xs font-medium text-emerald-800">이번 통화 메모</span>
          <textarea
            value={draft}
            onChange={(e) => setDraft(e.target.value)}
            rows={3}
            placeholder="통화 중 메모 (고객 SMS와 별도)"
