...
export function OrderFormPage({ editor }: { editor?: OrderFormEditorContext } = {}) {
  ...
  const [staffFillRules, setStaffFillRules] = useState<OrderFormFillRules>(DEFAULT_ORDER_FORM_FILL_RULES);
  ...
  const submitResumeAfterTimeSlotAckRef = useRef(false);
  ...
  const confirmServiceDateAck = useCallback(() => {
    ...
    setPendingServiceDate(null);
    setServiceDateAckOpen(false);
    if (submitResumeAfterServiceDateAckRef.current) {
      submitResumeAfterServiceDateAckRef.current = false;
  ...
  const handleCustomerPreferredTimeChange = useCallback(
    (raw: string) => {
      if (!customerScheduleAckEnabled) {
        handleEditorPreferredTimeChange(raw);
        return;
      }
      if (raw === '') {
        setTimeSlotConsent(null);
  ...
  const confirmTimeSlotAck = useCallback(() => {
    ...
    setPendingTimeSlot(null);
    setTimeSlotAckOpen(false);
    if (submitResumeAfterTimeSlotAckRef.current) {
      submitResumeAfterTimeSlotAckRef.current = false;
      resumeSubmitAfterAckRef.current = true;
    }
  }, [pendingTimeSlot]);
  ...
  useEffect(() => {
    if (!customerScheduleAckEnabled) return;
  ...
  useEffect(() => {
    if (!isCreate || !editorAuthToken) return;
    let cancelled = false;
    getFormConfig(editorAuthToken)
      .then((cfg) => {
        if (cancelled) return;
        setStaffFillRules(mergeOrderFormFillRules(cfg.issueFillRules));
      })
      .catch(() => {
        if (!cancelled) setStaffFillRules(DEFAULT_ORDER_FORM_FILL_RULES);
      });
    return () => {
      cancelled = true;
    };
  }, [isCreate, editorAuthToken]);
  ...
  useEffect(
    () =>
      subscribeOrderGuideAgreeTerms((payload) => {
        setGuideTermsConsent({ at: payload.agreedAt, signaturePng: payload.signaturePng });
      }),
    [],
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (stdFieldOn('areaPyeong') || areaLockedByAdmin) {
        if (!customerMayEditFillKey(order, 'areaPyeong')) {
          if (areaLockedByAdmin) {
            submitAreaPyeong = order!.areaPyeong!;
            submitAreaBasis = String(order!.areaBasis).trim();
          } else if (fillKeyRequiredForCustomer(order, 'areaPyeong')) {
            addIssue('상담사에게 면적(공급·전용)을 확인해 주세요. 고객이 임의로 고를 수 없습니다.', 'order-field-area');
          }
        } else if (areaLockedByAdmin) {
          submitAreaPyeong = order!.areaPyeong!;
          submitAreaBasis = String(order!.areaBasis).trim();
      ...
      if (!guideTermsConsent?.signaturePng) {
        addIssue('[필수] 안내사항을 끝까지 읽고 서명해 주세요.', 'order-field-agree');
      }
      ...
      if (
        ...
        submitResumeAfterServiceDateAckRef.current = true;
      ...
      const submitConsents: OrderFormSubmissionConsents = {
        guideTerms: guideTermsConsent
          ? { agreedAt: guideTermsConsent.at, signaturePng: guideTermsConsent.signaturePng }
          : undefined,
      };
  ...
  const handleCreateAndPrefill = async () => {
    ...
    const basisOk = form.areaBasis === '공급' || form.areaBasis === '전용';
    const areaFlags = fillRuleOf(staffFillRules, 'areaPyeong');
    if (stdFieldOn('areaPyeong') && (marketerMustFillAtIssue(areaFlags) || (canMarketerWrite(areaFlags) && (basisOk || form.areaPyeong.trim())))) {
      if (marketerMustFillAtIssue(areaFlags) || basisOk || form.areaPyeong.trim()) {
        if (!basisOk) {
    ...
    const dateFlags = fillRuleOf(staffFillRules, 'preferredDate');
    if (stdFieldOn('preferredDate') && !form.preferredDate.trim() && !canCustomerWrite(dateFlags)) {
      showSubmitError('청소 날짜를 선택해 주세요.', 'order-field-schedule');
      return;
    }
  ...
  if (!isEditor && customerWizard.currentStep) {
    const shared: CustomerWizardShared = {
      ...
      submitting,
      guideTermsAt: guideTermsConsent?.at ?? null,
      guideTermsSigned: Boolean(guideTermsConsent?.signaturePng),
          ...
          <div>
            ...
            {scheduleLockedByAdmin ? (
              ...
              <select
                ...
                onChange={(e) => handleCustomerPreferredTimeChange(e.target.value)}
              >
