...
export type EContractSubmissionRow = {
  ...
  recipientRole?: 'TEAM_LEADER' | 'MARKETER' | 'TEAM_MEMBER' | 'ADMIN' | 'EXTERNAL_PARTNER';
};

export type EContractTeamMemberPicker = {
  id: string;
  name: string;
  phone: string | null;
...
export type EContractSubmissionDetailDto = {
  ...
  definitionId: string;
  definitionTitle: string;
  teamLeader: { id: string; name: string; email: string; role?: 'TEAM_LEADER' | 'MARKETER' | 'ADMIN' | 'EXTERNAL_PARTNER' } | null;
...
export async function pickerTeamMembers(token: string): Promise<{ teamMembers: EContractTeamMemberPicker[] }> {
  const res = await fetch(`${API}/admin/e-contracts/pickers/team-members`, {
    headers: headers(token),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
  ...
  return res.json() as Promise<{ teamMembers: EContractTeamMemberPicker[] }>;
}
