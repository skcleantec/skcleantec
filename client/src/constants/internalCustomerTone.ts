...
export const INTERNAL_CUSTOMER_TONE_OPTIONS: ReadonlyArray<{
  ...
  { value: 'SEVERE', imageSrc: INTERNAL_CUSTOMER_TONE_IMAGE_SRC.SEVERE, label: '극악 고객' },
...
const DISPLAY_TONES = new Set<InternalCustomerTone>(['GOOD', 'BAD', 'SEVERE', 'ELDERLY']);

export function normalizeInternalCustomerTone(raw: unknown): InternalCustomerTone {
  const s = String(raw ?? '').trim().toUpperCase();
  if (s === 'GOOD' || s === 'BAD' || s === 'SEVERE' || s === 'ELDERLY') return s;
  return 'NORMAL';
}
...
export function internalCustomerToneImageSrc(
  ...
  if (tone === 'GOOD' || tone === 'BAD' || tone === 'SEVERE' || tone === 'ELDERLY') {
...
export function internalCustomerToneHint(tone: InternalCustomerTone | null | undefined): string {
  ...
  if (tone === 'SEVERE') return '극악 고객 — 악성보다 심각한 내부 주의';
  if (tone === 'ELDERLY') return '어르신 — 전화로 정보 재확인 권장';
  return '';
}

/** API 전송 — GOOD·BAD·SEVERE·ELDERLY만, 미선택은 undefined */
export function internalCustomerToneForApi(
  tone: InternalCustomerTone | null | undefined,
): DisplayTone | undefined {
  if (tone === 'GOOD' || tone === 'BAD' || tone === 'SEVERE' || tone === 'ELDERLY') return tone;
  return undefined;
}
