/**
 * 영업 브랜드(OperatingCompany)별 취소·변경 위약금 정책 — `OperatingCompany.config.cancellationPolicy`
 */
...
export type OperatingCompanyCancellationPolicy = {
  enabled: boolean;
  /** 위약 없이 변경·취소 가능한 최소 일수 (예: 2 = 청소일 2일 전까지) */
...
export function createCancellationPolicyTierId(): string {
  ...
  return `${CANCELLATION_POLICY_TIER_ID_PREFIX}${rand}`;
}
...
export function formatCancellationDaysBeforeLabel(daysBefore: number): string {
  if (daysBefore <= 0) return '당일';
  if (daysBefore === 1) return '전일';
