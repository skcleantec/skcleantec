...
export async function verifyGoogleIdTokenCredentials(idTokenRaw: string) {
  const clientId = getGoogleOAuthClientId();
  if (!clientId) {
    throw new AuthSignupOAuthError(
      'Google 로그인이 아직 설정되지 않았습니다. 아이디·비밀번호로 로그인해 주세요.',
      503,
    );
  }
...
export async function verifyGoogleSignupIdToken(idTokenRaw: string) {
  ...
  const { sub, providerEmail } = await verifyGoogleIdTokenCredentials(idTokenRaw);
