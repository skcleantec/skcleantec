...
import {
  findQuotationForTeamLeader,
  isInquiryAssignedToTeamLeader,
  teamQuotationListWhere,
} from './quotationTeamAccess.js';
...
function requireTeamLeader(req: import('express').Request, res: import('express').Response): AuthPayload | null {
  ...
  if (auth.role !== 'TEAM_LEADER') {
    res.status(403).json({ error: '자사 팀장만 이용할 수 있습니다.' });
    return null;
  }
  return auth;
}
...
router.get('/editor-defaults', async (req, res) => {
  const auth = requireTeamLeader(req, res);
  if (!auth) return;
...
router.get('/', async (req, res) => {
  ...
  if (inquiryIdRaw) {
    if (!(await isInquiryAssignedToTeamLeader(tenantId, inquiryIdRaw, auth.userId))) {
  ...
  const where: import('@prisma/client').Prisma.QuotationWhereInput = teamQuotationListWhere(
    tenantId,
    auth.userId,
  );
  if (inquiryIdRaw) {
...
router.get('/:id', async (req, res) => {
  ...
  const row = await prisma.quotation.findFirst({
    where: {
      id: req.params.id,
      ...teamQuotationListWhere(tenantId, auth.userId),
    },
    include: quotationInclude,
  });
  if (!row) {
...
router.patch('/:id', async (req, res) => {
  ...
  const { id } = req.params;
  const existing = await findQuotationForTeamLeader(tenantId, id, auth.userId);
  if (!existing) {
    res.status(404).json({ error: '견적서를 찾을 수 없습니다.' });
    return;
  }
    ...
    const nextId =
      nextRaw === null || nextRaw === ''
        ...
        : typeof nextRaw === 'string'
          ? nextRaw.trim()
          : 'INVALID';
    if (nextId === 'INVALID' || nextId !== existing.inquiryId) {
      res.status(400).json({ error: '연결 접수는 변경할 수 없습니다.' });
      return;
    }
  }
  ...
  const nextVatMode =
    body.vatMode !== undefined
      ? parseQuotationVatMode(body.vatMode)
      : (existing.vatMode as import('./quotationVat.js').QuotationVatMode);
  ...
  if (nextStatus === 'FINALIZED' || nextStatus === 'SENT') {
    try {
      ...
      console.error('[team-quotation] pdf store on finalize failed', e);
    }
  }
...
router.get('/:id/email-logs', async (req, res) => {
  ...
  const { id } = req.params;
  const exists = await findQuotationForTeamLeader(tenantId, id, auth.userId);
  if (!exists) {
    res.status(404).json({ error: '견적서를 찾을 수 없습니다.' });
    return;
  }
...
router.get('/:id/pdf', async (req, res) => {
  ...
  try {
    ...
    console.error('[team-quotation-pdf] failed', e);
