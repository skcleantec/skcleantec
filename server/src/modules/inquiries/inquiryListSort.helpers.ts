...
/** 필터 조건 전체에 목록 정렬 규칙을 적용한 뒤 페이지 slice — 페이지네이션과 미제출 최상단을 동시에 만족 */
export async function fetchInquiryListPageSorted<TInclude extends Prisma.InquiryInclude>(
  ...
  const sortRows = await db.inquiry.findMany({
    where: args.where,
    select: {
      id: true,
      status: true,
  ...
  const sorted = sortInquiryListSortables(sortRows);
  const total = sorted.length;
  const pageIds = sorted.slice(args.skip, args.skip + args.take).map((r) => r.id);
  if (pageIds.length === 0) {
    return { items: [], total };
  }
