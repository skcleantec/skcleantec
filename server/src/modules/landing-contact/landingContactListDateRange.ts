/** C/S 목록 — 접수일(createdAt) 기준, KST (landing contact 동일) */
...
export function landingContactCreatedAtRangeFromQuery(query: {
  datePreset?: string;
  month?: string;
  day?: string;
}): { gte: Date; lte: Date } {
  const preset = (query.datePreset ?? 'last3months') as LandingContactListDatePreset;
  if (preset === 'last3months') return kstLast3MonthsRange();
  if (preset === 'month') {
