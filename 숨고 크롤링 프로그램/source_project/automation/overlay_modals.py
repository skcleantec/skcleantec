...
function scoreDismiss(btn, rootRect) {
  ...
  var label = norm((btn.getAttribute('aria-label') || '') + ' ' + (btn.textContent || '') + ' ' + (btn.getAttribute('title') || ''));
  var score = 0;
...
function clickExactClose(root) {
  if (!root) return false;
  var nodes = root.querySelectorAll('button, a, [role="button"], span, p, div');
  var best = null;
  var bestScore = -1;
  for (var i = 0; i < nodes.length; i++) {
    ...
    var r = el.getBoundingClientRect();
    var score = 0;
    if (el.tagName === 'BUTTON' || el.tagName === 'A' || el.getAttribute('role') === 'button') score += 40;
    if (r.width < 200 && r.height < 64) score += 25;
    score += Math.min(r.top, 500) * 0.04;
...
if (!root) return false;
if (clickExactClose(root)) return true;
