...
<#
  ...
  (바탕화면·임의 폴더 설치 — AppDir/ExePath 는 실행 중인 EXE 기준으로 전달)
...
function Show-UpdateError([string]$Message) {
    ...
    try {
        ...
        [System.Windows.Forms.MessageBox]::Show(
            ...
            "$Message`n`n설치 위치: $AppDir`n`n자세한 내용: $LogPath",
...
function Copy-WithRetry([string]$Source, [string]$Destination, [int]$Attempts = 5) {
    ...
    for ($i = 1; $i -le $Attempts; $i++) {
        ...
        try {
            ...
            Write-UpdateLog "copy retry $i/$Attempts : $Source -> $Destination ($($_.Exception.Message))"
    ...
    throw "파일 복사 실패: $Source ($($lastError.Exception.Message))"

}



function Copy-UpdatePayload([string]$PayloadRoot, [string]$DestDir) {
    ...
    Copy-WithRetry (Join-Path $PayloadRoot 'SoomgoAutomation.exe') (Join-Path $DestDir 'SoomgoAutomation.exe')



    $helperSrc = Join-Path $PayloadRoot 'apply_zip_update.ps1'

    if (Test-Path -LiteralPath $helperSrc) {

        Copy-WithRetry $helperSrc (Join-Path $DestDir 'apply_zip_update.ps1')

    }



    $srcInternal = Join-Path $PayloadRoot '_internal'

    $dstInternal = Join-Path $DestDir '_internal'

    if (-not (Test-Path -LiteralPath $srcInternal)) {

        throw 'ZIP 안에 _internal 폴더가 없습니다.'

    }

    if (-not (Test-Path -LiteralPath $dstInternal)) {

        New-Item -ItemType Directory -Path $dstInternal -Force | Out-Null

    }



    $configPath = Join-Path $dstInternal 'config.json'

    $savedConfig = $null

    if (Test-Path -LiteralPath $configPath) {

        $savedConfig = Join-Path $env:TEMP ("soomgo-config-" + [guid]::NewGuid().ToString('N') + '.json')

        Copy-Item -LiteralPath $configPath -Destination $savedConfig -Force

        Write-UpdateLog '기존 config.json 백업'

    }



    Write-UpdateLog '_internal 동기화 (robocopy /E, MIR 아님 — images 등 유지)'

    & robocopy $srcInternal $dstInternal /E /NFL /NDL /NJH /NJS /NC /NS /NP /R:3 /W:2 | Out-Null

    $rc = $LASTEXITCODE

    Write-UpdateLog "robocopy _internal exit=$rc"

    if ($rc -ge 8) {

        throw "robocopy _internal 실패 (exit=$rc)"
    ...
    if ($savedConfig -and (Test-Path -LiteralPath $savedConfig)) {
        ...
        Write-UpdateLog 'config.json 복원'
...
function Start-AppProcess([string]$Exe, [string]$WorkDir) {

    Write-UpdateLog "재시작 시도: $Exe (cwd=$WorkDir)"

    try {

        Start-Process -FilePath $Exe -WorkingDirectory $WorkDir -ErrorAction Stop | Out-Null

        Write-UpdateLog 'Start-Process 성공'

        return

    } catch {

        Write-UpdateLog "Start-Process 실패: $($_.Exception.Message)"
    ...
    try {
        ...
        $arg = '/c start "" "' + $Exe.Replace('"', '""') + '"'

        Start-Process -FilePath 'cmd.exe' -ArgumentList $arg -WorkingDirectory $WorkDir -WindowStyle Hidden -ErrorAction Stop | Out-Null

        Write-UpdateLog 'cmd start 성공'

        return

    } catch {

        Write-UpdateLog "cmd start 실패: $($_.Exception.Message)"

    }



    throw "프로그램 재시작 실패: $Exe"
...
try {
    ...
    try {
        ...
        Copy-UpdatePayload -PayloadRoot $payloadRoot -DestDir $AppDir
        ...
        Start-AppProcess -Exe $ExePath -WorkDir $AppDir
