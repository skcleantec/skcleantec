...
|----------------|-----------------------------------|
| **새 기능·화면** | **먼저** 타 앱 조사 → 코드 점검 + UI + **화면 표시·설정** + 역할별 테스트 (+ DB/플랫폼 해당 시) |
| **버그 수정** | (화면이 바뀌면 타 앱 조사) + 코드 점검 + 회귀 테스트 (+ UI 바꿨으면 표시·설정 점검) |
...
| 이름 | 하는 일 |
|------|---------|
| **AppScout** | **맨 앞.** 타 앱 최신 작동방식·인기 모바일 디자인 웹검색 |
| **CodeGuardian** | 코드·룰·연관 파일·타입 검사 |
| **DesignPulse** | AppScout 레퍼런스를 PC·모바일 UI에 적용 |
