...
| 항목 | 결정 |
|------|------|
| 구매 전 금액 | **고객 현장 수금(잔금)** 과 **정보공유 수수료**를 **분리** 표시. 재판매 시 앞선 판매 수수료 + 이번 판매 수수료 = 구매자 부담 총 수수료 |
| 구매 전 PII | 이름 마스킹, 주소 시·구, 전화·이메일 비노출, **청소·일정 필드 전부 노출** (건축물·면적·구조·예약·특이사항·메모 등) |
| **회사 매출 vs 현장 수금** | **고객 총액·잔금(현장 수금)** 과 **업체 매출(정산·급여·대시보드)** 은 별도. 정보공유 건은 서비스 총액 전체를 매출로 잡지 않는다 — 아래 표 |
...
| 역할 | 시점 | 회사 매출 | UI 라벨 예 |
|------|------|-----------|------------|
| **최초 DB 판매** (hop 0) | **인계 확정** | 예약금 + 이번 hop 수수료 | 정보공유 예약금+수수료 |
| **재판매** (hop > 0) | 인계 확정 | 이번 hop 수수료만 | 정보공유 수수료 |
| **구매·시공** (파트너 mirror / 타업체) | 현장 기준 | **고객 잔금 − 수수료 총액** (수수료는 판매 업체에 지급) | 정보공유 잔금−수수료 |

- 계산 단일 소스: `shared/dbMarketplaceAmount.ts` (`computeMarketplaceSellerRevenue`, `computeMarketplaceBuyerCompanyRevenue`)
- **과거 CONFIRMED 건 backfill 없음** — 위 규칙은 **적용 이후** 새로 인계·집계되는 건부터 사용
- 파트너·타업체 **정산 미수/지급**은 기존처럼 `transferFee` / `externalTransferFee`(수수료) 기준이며, 표의 **매출** 열은 참고용으로 별도 표시
